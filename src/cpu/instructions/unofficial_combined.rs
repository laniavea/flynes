...
impl Cpu {
    ...
    pub fn op_alr(&mut self, data_ref: &u8) {
        self.reg_a &= data_ref;
    ...
    pub fn op_anc(&mut self, data_ref: &u8) {
        self.reg_a &= data_ref;
        update_zero_and_neg_flags(&mut self.cpu_status, self.reg_a);
    ...
    pub fn op_arr(&mut self, data_ref: &u8) {
        self.reg_a &= data_ref;
        self.reg_a >>= 1;
    ...
    pub fn op_axs(&mut self, data_ref: &u8) {
        let temp_and_result = self.reg_x & self.reg_a;
        set_flag(&mut self.cpu_status, CARRY_FLAG, temp_and_result >= *data_ref);
        self.reg_x = temp_and_result.wrapping_sub(*data_ref);
