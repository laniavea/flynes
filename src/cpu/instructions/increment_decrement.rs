...
fn test_incerement_decrement() {
    ...
    for _ in 0..1000 {
        ...
        for now_id in &random_ids {
            cpu.op_inc(&mut bus, *now_id as u16)
        }
        assert_eq!(bus.memory().ram().iter().sum::<u8>(), random_v);

        for now_id in &random_ids {
            cpu.op_dec(&mut bus, *now_id as u16)
        }
        assert_eq!(bus.memory().ram().iter().sum::<u8>(), 0);
