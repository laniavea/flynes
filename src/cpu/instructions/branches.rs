...
impl Cpu {
    ...
    pub fn op_bcs(&mut self, data_ref: &u8) {
        if is_flag_set(&self.cpu_status, CARRY_FLAG) {
            let relative_displacement = (*data_ref as i8) as i16;
    ...
    pub fn op_bcc(&mut self, data_ref: &u8) {
        if !is_flag_set(&self.cpu_status, CARRY_FLAG) {
            let relative_displacement = (*data_ref as i8) as i16;
            self.program_counter = self.program_counter.wrapping_add_signed(relative_displacement)
        }
    ...
    pub fn op_beq(&mut self, data_ref: &u8) {
    ...
    pub fn op_bne(&mut self, data_ref: &u8) {
        if !is_flag_set(&self.cpu_status, ZERO_FLAG) {
            let relative_displacement = (*data_ref as i8) as i16;
            self.program_counter = self.program_counter.wrapping_add_signed(relative_displacement)
        }
    ...
    pub fn op_bmi(&mut self, data_ref: &u8) {
    ...
    pub fn op_bpl(&mut self, data_ref: &u8) {
        if !is_flag_set(&self.cpu_status, NEGATIVE_FLAG) {
            let relative_displacement = (*data_ref as i8) as i16;
            self.program_counter = self.program_counter.wrapping_add_signed(relative_displacement)
        }
    ...
    pub fn op_bvs(&mut self, data_ref: &u8) {
    ...
    pub fn op_bvc(&mut self, data_ref: &u8) {
        if !is_flag_set(&self.cpu_status, OVERFLOW_FLAG) {
            let relative_displacement = (*data_ref as i8) as i16;
            self.program_counter = self.program_counter.wrapping_add_signed(relative_displacement)
        }
