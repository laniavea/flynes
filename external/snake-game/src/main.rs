...
const SNAKE_GAME_WIDTH: u32 = 32;
const SNAKE_GAME_HEIGHT: u32 = 32;
const SNAKE_GAME_WIDTH_F32: f32 = 32.0;
const SNAKE_GAME_HEIGHT_F32: f32 = 32.0;
const SCALE_WIDTH: f32 = 30.0;
const SCALE_HEIGHT: f32 = 30.0;
const ROM_START: usize = 0x0600;

fn main() {
    pretty_env_logger::init();

    let snake_game_hex: Vec<u8> = vec![
    ...
    let window = video_subsystem
        .window(
            "Snake game",
            (SNAKE_GAME_WIDTH_F32 * SCALE_WIDTH) as u32,
            (SNAKE_GAME_HEIGHT_F32 * SCALE_HEIGHT) as u32
        )
        .position_centered()
        .build()
        .unwrap();
    ...
    canvas.set_scale(SCALE_WIDTH, SCALE_HEIGHT).unwrap();

    let creator = canvas.texture_creator();
    let mut texture = creator
        .create_texture_target(PixelFormatEnum::RGB24, SNAKE_GAME_WIDTH, SNAKE_GAME_HEIGHT).unwrap();

    
    let mut screen_state = [0u8; (SNAKE_GAME_WIDTH * 3 * SNAKE_GAME_HEIGHT) as usize];
    ...
    loop {
        ...
        thread::sleep(Duration::new(0, 30_000));
    }
}
